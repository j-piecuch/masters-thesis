...
supply those characters. Most operating systems systems choose the buffering
...
system call to be restarted if it turns out that no signal has been caught. This
special return value is explained in more detail in
Section~\ref{chap:sleep-stop}.
...
usually distinguished: \textit{interruptible} and
...
widely used production-grade OS. The second bug can lead to a kernel panic (i.e.
a system crash), while the last one allows any process with write access to a
terminal to prevent all other processes in the system from writing to that
