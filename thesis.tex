...
sends data typed on the keyboard to the computer, and prints the response on a
piece of paper.
...
video terminal's screen. \Verb{xterm} is an example of a terminal emulator.
...
\end{minted}
\caption{\Verb{include/sys/proc.h}: definition of \Verb{pgrp_t}.}
\label{lst:pgrp}
\end{listing}
...
\end{minted}
\caption{\Verb{include/sys/signal.h}: definition of \Verb{sigaction_t}.}
\end{listing}
...
\begin{itemize}
\item The terminal device layer;
...
% This should go in the first chapter, introducing the concept of terminals.

The following discussion concentrates on terminal devices as a concept inside
the kernel, not hardware terminal devices.

A terminal device in the kernel represents an abstract hardware terminal. The
terminal being represented may or may not be an actual hardware device: a
terminal can be emulated in software using a \textit{terminal emulator}, which
uses pseudoterminals under the hood. In fact, hardware terminals are now used
very rarely, and terminal emulators are the most common use case for terminals
on desktop systems.

Terminal devices provide a layer of processing between processes and raw serial
devices. This processing might seem unnecessary at first, but its usefulness
very quickly becomes apparent.
...
The preceding example illustrates just one useful feature of terminal devices. 
...
On the surface, a terminal device is no different than a standard character
device. To interact with it, a process can open the corresponding device file
using \Verb{open()} and then perform input and output using \Verb{read()} and
\Verb{write()} respectively. The terminal interface specified by POSIX is much
richer than that. Terminal devices play a significant role in job control, and
therefore are connected to the concepts of process groups and sessions. We will
now examine the most important aspects of the terminal device specification
\cite{terminal-spec} found in POSIX and see how terminal devices tie into job
control.
...
which it is buffered in the \textit{input queue}. Processes calling
...
after processing, the data may be written directly to the underlying hardware
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/term-hilevel}
  \caption{A high-level overview of terminal data flow, based on \cite[Figure~62-1]{tlpi}}
  \label{fig:term-hilevel}
\end{figure}

\subsubsection{Controlling terminals}

A terminal device may be designated as the controlling terminal of a session. A
session may have at most one controlling terminal, and a terminal device may be
the controlling terminal of at most one session. A process can always access its
...
\begin{figure}[h]
  ...
  \caption{Proces hierarchy from figure~\ref{fig:proc-hierarchy} with
    ...
    sessions 1 and 2 do not have a controlling terminal. The terminal device
...
\Verb{VKILL} (which erases the whole buffer). These special characters are
processed by the operating system and are not forwarded to userspace. Once the
user types a \textit{line-terminating} character, the contents of the line
buffer are appended to the input queue and become available as input to
userspace programs.
...
special to the OS and appear as input on the terminal device.
...
For each terminal device, almost all of its settings, including the terminal
mode, are stored in a \Verb{termios} structure associated with that terminal.
The only exception is the terminal window size, which isn't a member of the
...
\subsection{Terminal devices in the Mimiker kernel}\label{chap:mimiker-tty}

We will now describe the implementation of the terminal device layer in the
Mimiker kernel.

\subsubsection{Overall design}

The terminal device layer provides the abstraction of a terminal device to
userspace processes, as well as to the rest of the kernel. It contains code that
is independent of the underlying hardware terminal device. In the subsystem
hierarchy, it sits below the generic file handling layer and above hardware
...
% TODO diagram prezentujący przepływ sterowania przy pisaniu znaków
% i czytaniu:
% Pisanie: proces --(write())--> system call layer
%                 --(f_write == tty_write())--> terminal layer
%                 --(tty_notify_out())--> UART terminal driver
%                 --(uart_putc())--> UART hardware
%
% Czytanie: proces --(read())--> system call layer
%                  --(f_read == tty_read())--> terminal layer: sleep or return
% Przychodzi znak: UART interrupt --(uart_intr())--> UART terminal driver
%                  --(tty_input())--> terminal layer --> notify sleeping threads
%                  about new data
...
associated with a single terminal device.
...
\end{minted}
\caption{\Verb{include/sys/tty.h}: definition of \Verb{tty_t}.}
\end{listing}
...
\item \Verb{TF_OUT_BUSY}: a thread is currently writing to this terminal.
  This flag is used to serialize \Verb{write()} calls on a terminal device.
\item \Verb{TF_IN_HIWAT}: the \textit{input high watermark} has been reached.
  Once space becomes available in the input queue, the hardware terminal driver
  will be notified.
\item \Verb{TF_DRIVER_DETACHED}: the hardware terminal driver detached from this
  terminal, usually because the device is no longer available. Any further I/O
  on this terminal should return an error.
\end{itemize}

\Verb{t_inq} is a ring buffer storing input coming from the hardware terminal
driver. \Verb{read()} calls on the terminal file descriptor receive characters
from this buffer. \Verb{t_incv} is a condition variable used to wait for input
to become available in \Verb{t_inq}.
...
The terminal device driver reads characters from this buffer and takes care of
...
its contents are copied into \Verb{t_inq}. The contents of \Verb{t_inq}
cannot be changed. In non-canonical mode, \Verb{t_line} is not used.
...
\Verb{t_ops} is a structure containing implementations of terminal device
driver operations. There are currently four operations that a driver can supply,
...
\end{minted}
\caption{\Verb{include/sys/tty.h}: definition of \Verb{ttyops_t}.}
\end{listing}
The \Verb{t_notify_out} function notifies the device driver that new data has
appeared in the terminal structure's output queue (i.e. \Verb{t_outq}). The
driver should ensure that data from the queue is written out to the device as
soon as possible. It is the only required function.

The \Verb{t_notify_in} function notifies the device driver that space has become
...
\Verb{t_vnode} points to the filesystem node representing the terminal
device. It is used to implement the \Verb{/dev/tty} device file, which refers
different devices for processes in different sessions. The \Verb{t_opencount}
field keeps track of the number of open file descriptors for the terminal.

\Verb{t_data} is an opaque pointer for the underlying hardware terminal device
driver to store its private data.

\subsubsection{Lifecycle of a terminal device}

When a terminal device driver attaches to a device, it is responsible for
creating its corresponding kernel terminal device. This involves allocating a
new \Verb{tty_t} structure, setting device-specific fields in that structure
(e.g. \Verb{t_ops}) and creating a terminal device node in the \Verb{/dev}
directory of the filesystem.

The \Verb{tty_makedev()} function takes care of creating the device filesystem
node. When creating the file, the function supplies \Verb{tty_fileops} to the
file-handling layer as the implementation of file operations. This way,
\Verb{read()} and \Verb{write()} (and other) calls on the file are directed to
the terminal layer, which may in turn call the terminal device driver via
\Verb{t_ops}.

Once a terminal device is created and visible in the filesystem, processes can
open and perform I/O on the device.

The process of deleting a terminal device begins when the driver detects that
the underlying hardware device is no longer available (e.g. due to a modem
disconnect). The driver then \textit{detaches} itself from the in-kernel
terminal device structure. After that happens, processes are no longer able open
or perform I/O on the terminal device file. Once the last open file descriptor
referencing the terminal is closed, the terminal structure's memory is returned
to the kernel.
...
case of terminal device files, the \Verb{fo_read} function pointer is set to the
\Verb{tty_read()} function.
...
\end{minted}
\caption{\Verb{kern/tty.c}: simplified definition of \Verb{tty_do_read()}.}
\label{lst:tty-do-read}
\end{listing}

The \Verb{f} parameter points to a \Verb{file_t} structure representing an open
file. The \Verb{uio} parameter describes the I/O operation and contains a
pointer to the destination buffer (or buffers).
...
\Verb{t_incv} condition variable.
...
\cite[Section~11.1.7]{terminal-spec}), but it works for most applications using
this mode, such as text editors.
...
It is the responsibility of the terminal device driver to pass characters
arriving at the hardware device to the terminal layer. The driver does this by
invoking the \Verb{tty_input()} function on the terminal structure, supplying
the new character. The function takes care of all input processing and putting
...
\end{minted}
\caption{\Verb{kern/tty.c}: simplified control flow of \Verb{tty_input()}.}
\end{listing}
...
\end{minted}
\caption{\Verb{kern/tty.c}: simplified definition of \Verb{tty_write()}.}
\label{lst:tty-write}
\end{listing}
...
The \Verb{tty_do_write()} function puts each character in the buffer supplied by
the process into the output queue using \Verb{tty_output_sleep()}. At the end,
it notifies the terminal device driver of new data using
\Verb{tty_notify_out()}.

The \Verb{tty_output_sleep()} function puts characters into the output queue
using \Verb{tty_output()}, sleeping on the \Verb{t_outcv} condition variable if
there is no space left. The condition variable is signalled when the driver
makes enough space available in the output queue. It calls \Verb{tty_output()}
on each character to process it and insert it into the output queue. Let us take
a look at the code of \Verb{tty_output()}.
...
\end{minted}
\caption{\Verb{kern/tty.c}: simplified definition of \Verb{tty_output()}.}
\label{lst:tty-write}
\end{listing}
...
terminal flags. During processing, a single character can be converted into
multiple characters, which is why we use the \Verb{cb} buffer to store them, and
\Verb{ccount} is the number of resulting characters after processing.
...
session's leader opens a terminal device file. The association happens only when
the session doesn't already have a controlling terminal, and the terminal being
...
and the UART device is shown in figure \ref{fig:term-hilevel-uart}.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/term-hilevel-uart}
...
\end{minted}
\caption{\Verb{include/sys/uart_tty.h}: definition of \Verb{tty_thread_t}.}
\end{listing}
...
\end{minted}
\caption{\Verb{include/sys/uart.h}: definition of \Verb{uart_state_t}.}
\end{listing}
...
\end{minted}
\caption{\Verb{sys/kern/uart_tty.c}: definition of \Verb{uart_tty_fill_txbuf()}.}
...
characters directly from the output queue to the hardware.
...
The last step in the function is calling \Verb{tty_getc_done()}. In the case of
terminal drivers that output characters asynchronously, the driver is
...
\end{minted}
\caption{\Verb{sys/kern/uart_tty.c}: definition of \Verb{uart_tty_thread()}.}
\end{listing}
...
\end{minted}
\caption{\Verb{sys/kern/uart.c}: definition of \Verb{uart_intr()}.}
\end{listing}
...
exposes a terminal device file that behaves in the same way as a normal terminal
device file would. Processes can open it and perform I/O on it. The terminal
emulator plays the role of the hardware device: it translates the user's
keystrokes into characters that are then made available as input to the
processes reading from the emulated terminal device. It also reads the
characters output by processes and displays them in the emulated terminal window.
...
terminal device. The master device is used to emulate the hardware terminal.
...
directory. The file path of the slave device corresponding to a master device
can be retrieved using the \Verb{ptsname()}\cite{ptsname} function.
...
from the filesystem, and the effect is as though the emulated hardware device
...
simple, as most of the logic is implemented in the terminal layer. All master
device logic is implemented as a terminal driver and plugs into the terminal
layer in the same way as the UART driver.
...
\end{minted}
\caption{\Verb{sys/kern/pty.c}: definition of \Verb{pty_t}.}
\end{listing}
...
data to become available in the slave terminal device's \textbf{output} queue,
while the \Verb{pt_outcv} condition variable is used by master-side writers to
wait for space to become available int the slave terminal device's
\textbf{input} queue.

\subsubsection{Master-side input}

Processes write to the master device of a pseudoterminal using the
\Verb{write()} system call, which inside the kernel uses the \Verb{fo_read}
...
\end{minted}
\caption{\Verb{sys/kern/pty.c}: definition of \Verb{pty_read()}.}
\end{listing}

The function first \ref{l1} loops until there is some data in the corresponding
slave terminal's output queue. If \ref{l2} the slave device isn't opened by any
processes, we return without reading anything. Otherwise, we \ref{l3} wait on
the pseudoterminal's \Verb{pt_incv} condition variable. If our sleep got
...
\end{minted}
\caption{\Verb{sys/kern/pty.c}: definition of \Verb{pty_notify_out()}.}
\end{listing}
...
\end{minted}
\caption{\Verb{sys/kern/pty.c}: definition of \Verb{pty_write()}.}
\end{listing}
...
the input queue, and also returns an error if the slave device isn't opened by
...
\end{minted}
\caption{\Verb{sys/kern/pty.c}: definition of \Verb{pty_notify_in()}.}
\end{listing}
...
\section{Interruptible sleep and stop signals}

When a process performs certain I/O operations (e.g. reading from a terminal
device), if the data isn't immediately available, the calling thread will
\textit{sleep} (i.e. suspend execution in the kernel) awaiting the data. Two
kinds of sleep are usually distinguished: \textit{interruptible} and
\textit{uninterruptible}. Interruptible sleep is used in case of I/O that may
take an arbitrarily long time to complete, as is the case when reading from a
terminal --- there is no guarantee that data will eventually arrive.
...
a signal, depending on the value of the handler's \Verb{SA_RESTART} flag:
\begin{itemize}
...
When a thread receives a stop signal (e.g. \Verb{SIGSTOP}), its execution should
...
\item A thread sleeping interruptibly can be indefinitely prevented from waking
  ...
  until it is resumed. A malicious program could use this technique to prevent
  all other processes from writing to a terminal device.
\end{itemize}
...
\item Since threads can stop only at the userspace boundary, they can't hold any
  exclusive kernel resource (such as exclusive access to a terminal device) when
  stopping.
\end{itemize}
...
\item \Verb{ERESTARTNOHAND}: restart the system call if and only if no signal
  was caught.
\end{itemize}
Linux defines two more error codes like this (\Verb{ERESTARTNOINTR} and
\Verb{ERESTART_RESTARTBLOCK}), but at the time of writing they are not needed to
implement any system call in the Mimiker kernel, so they have not been included.

Note that a signal is \textit{caught} only if its handler is executed. Therefore,
if a thread is stopped using \Verb{SIGSTOP} and resumed using \Verb{SIGCONT},
assuming \Verb{SIGCONT} had no associated handler, then no signal was caught.
Consequently, if a system call returns \Verb{ERESTARTSYS}, and the only signals
handled at the userspace boundary are \Verb{SIGSTOP} followed by \Verb{SIGCONT}
(without a handler), then the system call will be restarted in the same way it
would be if there were no signals to handle at all.

As a concrete example, let's trace what happens when a thread inside of a
\Verb{write()} system call on a terminal device attempts to sleep interruptibly
while having a pending \Verb{SIGSTOP} signal:
\begin{enumerate}
\item In \Verb{tty_do_write()} the thread calls \Verb{tty_output_sleep()}, which
  finds out the terminal output queue full and calls \Verb{tty_wait()} to sleep
  on the condition variable \Verb{t_outcv}.
\item The \Verb{cv_wait_intr()} function, used to sleep on the condition
  variable, calls \Verb{sleepq_wait_timed()}, which is the main kernel function
  used to go to sleep.
\item The \Verb{sleepq_wait_timed()} function checks the current thread's
  \Verb{TDF_NEEDSIGCHK} flag, which indicates that there is a pending signal. If
  the flag is set, the function immediately returns \Verb{EINTR}.
\item The \Verb{EINTR} error code is propagated to \Verb{tty_wait()}, which
  converts it to \Verb{ERESTARTSYS}.
\item The \Verb{ERESTARTSYS} error code is propagated through the call stack,
  during which the exclusive terminal access gained in \Verb{tty_write()} is
  relinquished.
\item Before returning to userspace, in \Verb{on_user_exc_leave()}, the
  \Verb{sig_check()} function checks for pending signals. Signals that should
...
\item The return value from \Verb{sig_check()}, along with the system call
  ...
  semantics of \Verb{ERESTARTSYS}, since no signal is to be caught, the system
  call is arranged to be restarted by appropriately modifying the userspace
  thread's context.
\item Upon return from the kernel to userspace, the \Verb{write()} system call
  is immediately restarted.
\end{enumerate}

\section{Refinement of locking rules}

As explained in section \ref{chap:locking-intro} (TODO: write the chapter we're
referring to here), locking primitives such as mutexes are used to synchronize
concurrent accesses to members of data structures that are shared between
threads. When using these locking primitives, care must be taken to prevent
\textit{deadlocks}. A deadlock occurs when a group of two or more threads is
stuck, with each thread in the group waiting for a resource that is held by
...
of the kernel (e.g. signal handling), the per-terminal \Verb{t_lock} embedded in
the \Verb{tty_t} structure had to be placed somewhere in the lock ordering. This
posed some problems, as the following example illustrates.

Consider what happens when a session leader terminates. If the session has an
...
terminal, the \Verb{all_proc_mtx} mutex must be held. In order to examine a
terminal's foreground process group, the terminal's \Verb{t_lock} mutex must be
held. Therefore, \Verb{all_proc_mtx} must be acquired first in order to look up
the controlling terminal, and then \Verb{t_lock} must be acquired to examine the
foreground process group. This places \Verb{all_proc_mtx} before \Verb{t_lock}
...
requires both \Verb{all_proc_mtx} and the target process group's \Verb{pg_lock}
to be held by the caller. This implies that \Verb{pg_lock} comes after
\Verb{t_lock} and \Verb{all_proc_mtx} in the ordering.
...
\Verb{t_lock}. In order to send a signal to the foreground process group, we
must acquire \Verb{all_proc_mtx} and the group's \Verb{pg_lock}. However, note
that acquiring \Verb{all_proc_mtx} while holding \Verb{t_lock} would violate the
ordering!
...
the \Verb{p_parent} field of the process receiving the signal. This field used
to be protected by \Verb{all_proc_mtx}, so that requirement also needed to be
changed. 

The first issue was resolved by using the parent process's \Verb{p_lock} to
synchronize the waiting thread with the notifying thread. The
