...
\englishabstract{Most applications with a text user interface require support
  ...
  from other systems compliant with the specification. To demonstrate the
  system's new capabilities, the \Verb{script} and \Verb{tetris} programs have
  been ported from NetBSD to the Mimiker OS.}
...
\polishabstract{Większość aplikacji posiadających tekstowy interfejs użytkownika
  ...
  specyfikacją. Aby zademonstrować nowe zdolności systemu, programy
  \Verb{script} oraz \Verb{tetris} zostały przeniesione z systemu NetBSD do
  systemu operacyjnego Mimiker.}
...
applications, such as shells. Two crucial features that were missing are
\textit{job control} and the \textit{terminal subsystem}. These have been
implemented by the author, and are the primary subject of this thesis. In
addition, several programs making use of these features have been ported to the
Mimiker OS by the author, chief among them the well-known game of Tetris. The
game makes use of advanced terminal features such as random cursor movement and
coloured output. It can also be used to show the system's job control
capabilities, such as the ability to stop the game's process by typing \Verb{^Z}
(Control-Z) on the keyboard. Another contribution made by the author is finding
three bugs in the terminal subsystem of the FreeBSD kernel. The FreeBSD
operating system\footnote{\url{https://www.freebsd.org/}} is a widely used
production-grade OS, used by companies such as Netflix and
Sony~\cite[Section~1.2.2]{freebsd-handbook}.

The structure of this thesis is as follows: this chapter provides an
introduction to job control, terminals, and the Mimiker kernel.
Chapter~\ref{chap:job-control} describes in detail the implementation of job
control, while Chapter~\ref{chap:tty} presents the terminal subsystem. Both
chapters begin with on overview of the relevant parts of the POSIX
specification. In Chapter~\ref{chap:impl-challenges} we dive into interesting
technical issues which arose while implementing the two primary features.
Chapter~\ref{chap:conclusion} concludes with a summary of the author's
contributions, potential directions for related future work in Mimiker, and
instructions for compiling and running Mimiker, should the reader want to try
it out themselves.
...
The \Verb{cat} and \Verb{grep} programs constitute a single
\textit{job}~\cite[Section~3.202]{posix-defs}. A job is group of programs
accomplishing a single logical task. In this case, the job
...
control running jobs~\cite[Section~3.203]{posix-defs}~\cite[Section~9.8]{apue}.
We will now discuss the most important use cases for job control which have been
made possible on Mimiker thanks to the implementation of missing functionality
by the author.

\subsubsection*{Foreground and background jobs}

The \Verb{&} symbol separating the two jobs in the example command makes the
first job run in the \textit{background}~\cite[Section~3.35]{posix-defs}, and
the second one in the \textit{foreground}~\cite[Section~3.184]{posix-defs}. Only
one job can be in the foreground at any time. Background jobs are restricted
from writing output and reading input from the terminal. While the foreground
job is running, the shell cannot accept new commands from the user, as it is
waiting for the foreground job to complete.
...
running. A background job can be put in the foreground using the
\Verb{fg}~\cite{fg-command} shell command, which will also resume the target job
if it was stopped.
...
kernel called \textit{process
  groups}~\cite[Section~3.296]{posix-defs}~\cite[Section~9.4]{apue}. When a
shell runs a new job, it puts all the processes in the job in the same process
group, so that they can be managed as a single unit. The discussion in
subsequent chapters does not mention jobs, focusing on process groups instead.
...
to shared data structures. Threads executing in the kernel (e.g.\ when
performing a system call) share the same address space and can access shared
kernel data structures. Therefore, synchronization is necessary inside the
kernel to preserve the consistency of these structures. In the Mimiker kernel,
this is accomplished using \textit{synchronization primitives}. The source code
listings found throughout this thesis contain frequent references to these
primitives, hence it is useful to have a basic understanding of their usage
before reading the kernel's code.
...
be suspended. This prevents events known as \textit{lost wake-ups} from
...
created using \Verb{fork()}\cite{fork}~\cite[Section~8.3]{apue}, it joins the
process group of its parent. A process can change its own process group, or that
of one of its children. This is done using the \Verb{setpgid()}\cite{setpgid}
function. It is used by the shell to set the process group of all processes in a
job.
...
inhabitant, also called the \textit{process group
  leader}~\cite[Section~3.298]{posix-defs}. The leader process is not special in
any way, other than the fact that its PID is equal to the PGID of the group it
is in.

Process groups are useful from a job control perspective, since certain POSIX
functions like \Verb{waitpid()}\cite{wait}~\cite[Section~8.6]{apue} and
\Verb{kill()}\cite{kill}~\cite[Section~10.9]{apue} can operate on entire process
groups. For instance, \Verb{waitpid()} allows the caller to wait for a status
change of any child process in the specified process group.

\subsubsection{POSIX sessions}

The concept of a
session~\cite[Section~3.343]{posix-defs}~\cite[Section~9.5]{apue} is fairly
intuitive. A new session usually starts when a user logs into the system.
Initially, the user's shell is the only process in the session. All jobs (and
therefore process groups) created by the shell belong to the same session. Every
process group must belong to exactly one session. Sessions are collections of
process groups, much in the same way as process groups are collections of
processes. A notable difference is that a process group cannot change its
session during its lifetime, while a process can change its process group. Like
processes and process groups, sessions have numeric identifiers called session
IDs, or SIDs.

A session may have an associated terminal device. That terminal device is called
the session's \textit{controlling
  terminal}~\cite[Section~3.115]{posix-defs}~\cite[Section~9.6]{apue}. They will
...
The process that creates a new session is called the \textit{session
  leader}~\cite[Section~3.344]{posix-defs}. Usually, the session leader a is
shell, or some other program ``in charge'' of running and controlling all other
programs in the session. It is supposed to be the last process in its session to
exit. If a session leader exits while its session contains other processes,
every process in the session will receive a \Verb{SIGHUP} signal, whose default
effect is to kill the receiving process. Processes can ignore this signal, so it
is possible for a session to outlive its leader.
...
This is where the concept of \textit{orphaned process
  groups}~\cite[Section~3.264]{posix-defs}~\cite[Section~9.10]{apue} comes into
play. A process group is orphaned when none of the processes in the group have a
parent that is in the same session, but in a different process group.
...
@\llabel{x1}@  SCOPED_MTX_LOCK(all_proc_mtx);
  proc_t *targetp = proc_find_raw(target);

@\llabel{x2}@  if (targetp == NULL || !proc_is_alive(targetp) ||
      (targetp != p && targetp->p_parent != p))
    return ESRCH;
@\llabel{x3}@  if (targetp == targetp->p_pgrp->pg_session->s_leader)
    return EPERM;
@\llabel{x4}@  if (targetp->p_pgrp->pg_session != p->p_pgrp->pg_session)
    return EPERM;

  pgrp_t *pg = pgrp_lookup(pgid);

  /* Create new group if one does not exist. */
@\llabel{x5}@  if (pg == NULL) {
...
@\llabel{x6}@  } else if (pg->pg_session != p->p_pgrp->pg_session) {
    /* Target process group must be in the same session
     * as the calling process. */
    return EPERM;
  }

@\llabel{x7}@  return _pgrp_enter(targetp, pg);
}
\end{minted}
...
First, we \ref{x1} acquire the \Verb{all_proc_mtx}, which synchronizes accesses
to process-tree structures, such as process groups and sessions. Next, lines
\ref{x2}, \ref{x3} and \ref{x4} respectively perform the following checks:
\begin{itemize}
...
\end{itemize}
We then \ref{x5} check whether the target process group exists. If it doesn't,
it is created, but only if the requested PGID matches the PID of the target
process. At \ref{x6} we perform yet another permission check. Finally, we call
...
\begin{itemize}
  ...
  \item The \Verb{all_proc_mtx} lock is already held everywhere
    ...
    be made in order to accommodate this rule.
\end{itemize}
...
The \Verb{s_hash} field is used to link the structure into the hash table used to
look up session structures by their session ID (SID).
...
@\llabel{z1}@  SCOPED_MTX_LOCK(&all_proc_mtx);
...
@\llabel{z3}@  pg = pgrp_create(pgid);
@\llabel{z4}@  pg->pg_session = session_create(p);

@\llabel{z5}@  return _pgrp_enter(p, pg);
}
\end{minted}
...
First, we~\ref{z1} acquire the \Verb{all_proc_mtx} lock, which is required to perform
things such as process group lookups and creating new sessions and process
groups.

At~\ref{z2}, we ensure that there doesn't already exist a process group with the
...
Once we have ensured that we can create the group, we do so at~\ref{z3}. Then,
we~\ref{z4} create a new session with \Verb{p} as the session leader and link it
to the newly created group (the session created by \Verb{session_create()} has a
\Verb{s_count} of 1, so there's no need to adjust it). Finally, we~\ref{z5}
...
\textit{Signals}~\cite[Section~3.350]{posix-defs}~\cite[Chapter~10]{apue} are
used to notify processes of various events. These events can occur
\textit{synchronously} or \textit{asynchronously} with respect to the process
...
process termination), or by other processes, using the \Verb{kill()}
function~\cite{kill}~\cite[Section~10.9]{apue}. A process can send a signal to
itself using the \Verb{raise()} function. As a shorthand, we shall say that a
signal is sent to a process group when it's sent to every process that is a
member of the group.
...
\textit{disposition}~\cite[Section~10.2]{apue}. There are three possible actions
that can be taken in response to a signal:
\begin{enumerate}
...
\item Invoke a \textit{signal handler routine}.
  ...
  The handler routine is run in the context of the receiving process. After the
  handler finishes execution, the process is resumed.
\end{enumerate}

The mapping of signal numbers to signal actions is controlled using the
\Verb{sigaction()} function~\cite{sigaction}~\cite[Section~10.14]{apue}. Not
every signal can have its action modified: \Verb{SIGKILL} and \Verb{SIGSTOP}
cannot have their actions changed from the default one, which is to terminate or
stop the receiving process, respectively.

The \textit{delivery} of a signal occurs when the target (i.e.\ receiving)
process takes the action associated with the signal~\cite[Section~10.8]{apue}. A
signal may be delivered long after it is initially sent, or it may not be
delivered at all, even if it isn't ignored. This is because a process may
\textit{block} a set of signals from being delivered. A blocked signal cannot be
delivered until it is unblocked. Contrary to signals that are ignored, blocked
signals await delivery instead of being discarded. A signal that is awaiting
delivery is said to be \textit{pending}.

The \textit{signal mask} determines the set of blocked signals for a thread. It
can be examined and modified using the \Verb{sigprocmask()}
function~\cite{sigprocmask}~\cite[Section~10.12]{apue}. Unsurprisingly, the
\Verb{SIGKILL} and \Verb{SIGSTOP} signals cannot be blocked from being
delivered.
...
process, every thread has its own set of blocked signals\footnote{This fact is
  relevant only on kernels which support processes with multiple threads. The
  Mimiker currently supports only single-threaded processes. }. This set is
stored in the \Verb{td_sigmask} field of the \Verb{thread_t} structure, which
describes a single thread of execution. The field is just a bit vector, with
each bit corresponding to a signal number. If a bit is set, signals with the
corresponding number are blocked from being delivered.
...
anymore. A return value of zero~\ref{b7} indicates to the caller that no signal
needs to be posted.
...
At the very end, we~\ref{d4} wake up the thread of the process we are
continuing. The \Verb{thread_continue()} function is very simple, and
is presented in Listing~\ref{lst:thread-continue}.
...
and output asynchronously relative to the \Verb{write()} call. Buffering is a
good idea in most cases, since the rate at which the terminal device is able to
accept characters is usually far slower than the rate at which the CPU can
supply those characters. Most operating systems choose the buffering
approach, and the same choice has been made in the Mimiker kernel.
...
A terminal may be designated as the controlling
terminal~\cite[Section~9.6]{apue} of a session. A session may have at most one
controlling terminal, and a terminal may be the controlling terminal of at most
one session. A process can always access its controlling terminal (if it has
one) by opening the \Verb{/dev/tty} device file. We define the controlling
terminal of a process to be simply the controlling terminal of that process's
session.
...
using the \Verb{tcsetpgrp()}\cite{tcsetpgrp}~\cite[Section~9.7]{apue} function.
Any process can set the foreground process group of its controlling terminal,
although some restrictions apply to processes from background process groups. A
controlling terminal does not need to have a foreground process group at all
times.
...
\begin{figure}[h]
  ...
  \caption{Process hierarchy from Figure~\ref{fig:proc-hierarchy} with
...
editing~\cite[Section~18.2]{apue}. The terminal mode has no influence over other
aspects of input processing, such as sending signals in response to certain
control characters.
...
the shell using the \Verb{stty} program~\cite{stty}~\cite[Section~18.6]{apue},
available on most UNIX-like operating systems, including Linux. To list the
current values of all terminal flags and control characters for the shell's
controlling terminal, use the command:
\begin{quote}
  \Verb{stty -a}
\end{quote}
For the full specification of the \Verb{termios} structure, see~\cite{termios}.
Implementations of POSIX may extend the available flags, control characters and
fields of the \Verb{termios} structure. For instance, \Verb{WERASE}
(\textit{word erase}) is an additional control character supported by many
kernels. These extensions are documented thoroughly in Sections~18.2 and 18.3 of \cite{apue}.
...
Listing~\ref{lst:tty-t} contains the structure's definition,
...
only \Verb{t_notify_out} is required:
\begin{listing}[H]
\begin{minted}{c}
...
descriptors for the terminal increases from zero to one. Predictably,
...
usually not allowed to read from their controlling terminal.
The~\ref{f2}~\Verb{tty_check_background()} function determines whether the
calling process can perform I/O on the given terminal. The \Verb{SIGTTIN}
argument specifies that we are trying to read from the terminal. If the function
determines that we are not allowed to continue with the operation, it will
return an error code. It may also send the specified signal (\Verb{SIGTTIN} in
this case) to every process in the calling process's process group.
...
Instead of being woken up by another thread signalling the condition variable,
it is also possible that we were woken up by receiving a signal that needs
handling. In other words, our sleep was \textit{interrupted} (that's what the
\Verb{intr} in \Verb{cv_wait_intr()} stands for --- \textit{interruptible}).
At~\ref{p3} we check whether our sleep was interrupted by a signal. If it was,
we return \Verb{ERESTARTSYS}, which is a special return value that will cause
the system call to be restarted if it turns out that no signal has been caught.
...
@\llabel{w1}@      if ((error = tty_check_background(tty, SIGTTOU)))
        return error;
    }

@\llabel{w2}@    while (tty->t_flags & TF_OUT_BUSY)
...
The function's primary role is to~\ref{w1} check whether we are allowed to
perform the operation using \Verb{tty_check_background()} and to serialize calls
to \Verb{tty_do_write()} by~\ref{w2} waiting until the \Verb{TF_OUT_BUSY} flag
is cleared.
...
check whether the low watermark has already been reached. If it hasn't,
we~\ref{o4} set the \Verb{TF_WAIT_OUT_LOWAT} flag to indicate that there is a
...
static bool tty_output(tty_t *tty, uint8_t c) {
  ...
  if (!(oflag & OPOST)) {
@\llabel{y1}@    return tty_outq_putc(tty, c);
  }

  uint8_t cb[2];
  cb[0] = c;
@\llabel{y2}@  int ccount = tty_process_out(tty, oflag, cb);
  int col = tty->t_column;

@\llabel{y3}@  if (!tty_outq_write(tty, cb, ccount))
    return false;

@\llabel{y4}@  for (int i = 0; i < ccount; i++) {
...
If the \Verb{OPOST} flag isn't set, we skip all output processing and~\ref{y1}
write the character directly to the output queue. Otherwise, we~\ref{y2} call
...
We then~\ref{y3} attempt to write the processed characters to the output queue.
...
We therefore~\ref{y4} loop over every character we have written to the output
...
\Verb{ioctl()}\cite[Section~3.15]{apue} is a very general, catch-all system call
that is most commonly used to provide functionality that is too simple to merit
a separate system call. It takes as arguments a file descriptor, an
\textit{opcode} (short for \textit{operation code}), and a generic argument. The
opcode is simply a numeric value that tells the kernel what operation to
perform. The argument's interpretation depends on the specific opcode. Most
opcodes are only valid for file descriptors of a certain type, e.g.\ ones
representing terminals. The opcodes used for getting and setting the foreground
process group are \Verb{TIOCGPGRP} and \Verb{TIOCSPGRP} respectively.
...
characters. In that case, if there are characters in the transmit buffer,
we~\ref{i3} tell the hardware to send an interrupt whenever it's ready to accept
...
Once we get a signal, we check what kind of work needs to be done. If the
\Verb{TTY_THREAD_RXRDY} flag is set, we~\ref{j2} read characters from the
receive buffer and~\ref{j3} pass them to the terminal layer using the
\Verb{tty_input()} function. If the \Verb{TTY_THREAD_TXRDY} flag is set,
we~\ref{j4} move characters from the terminal's output queue to the transmit
buffer using the \Verb{uart_tty_fill_txbuf()} function.
...
\end{minted}
\caption{\href{https://mimiker.ii.uni.wroc.pl/source/xref/mimiker/sys/drv/uart.c?r=2609772a\#18}{\Verb{sys/drv/uart.c}: definition of \Verb{uart_intr()}.}}
\end{listing}
...
while loop. After that, if~\ref{k6} we have emptied the transmit buffer,
we~\ref{k8} signal the worker thread to move some characters into the transmit
...
is called a
\textit{pseudoterminal}~\cite[Section~3.308]{posix-defs}~\cite[Chapter~19]{apue}.
A terminal emulator is far from the only use case for pseudoterminals. Programs
like \Verb{sshd} (an SSH server) or \Verb{script} (which records an interactive
terminal session into a file) also use pseudoterminals as a basic component
(see~\cite[Section~64.1]{tlpi} for a good explanation of how an SSH server uses
pseudoterminals). We will now take a look at how POSIX specifies their
operation.
...
\Verb{posix_openpt()}\cite{posix-openpt}~\cite[Section~19.3]{apue} function. It
returns an open file descriptor to the master device of a pseudoterminal. Note
that the master device isn't exposed in the filesystem\footnote{On BSD systems,
  pseudoterminal master devices are accessible via the
  filesystem~\cite[Section~64.8]{tlpi}.}: initially only the the process that
created the pseudoterminal has access to the master device (although it can
duplicate and transfer the file descriptor to another process).
...
@\llabel{l4}@  int error = ringbuf_read(&tty->t_outq, uio);
...
how it operates. Listing~\ref{lst:pty-read} presents the function's source code.
...
The \Verb{pt_incv} condition variable is signalled when new data arrives in the
...
buffer supplied by the calling process. Before reading the actual
character,~\ref{m2} the current status of the I/O operation is saved. We
then~\ref{m3} read the character to be written from the userspace buffer
and~\ref{m4} write it to the slave terminal's input queue using the
\Verb{pty_putc_sleep()} function, which sleeps on the \Verb{pt_outcv} condition
variable if there's no space in the input queue, and also returns an error if
the slave terminal isn't opened by any processes. Once there's space in the
input queue, the \Verb{t_notify_in} function pointer is invoked, which in our
case points to \Verb{pty_notify_in}:
\begin{listing}[H]
\begin{minted}{c}
...
character at~\ref{m3}, since we failed to properly consume it. To do that,
we~\ref{m5} restore the previously saved state of the I/O operation. After
rolling back, we break out of the loop and return with an error code.

\chapter{Implementation Challenges}\label{chap:impl-challenges}
...
The first issue concerns the way stop signals (e.g. \Verb{SIGSTOP}) interact
...
(i.e.\ suspend execution in the kernel) awaiting the data. Two kinds of sleep
are usually distinguished: \textit{interruptible} and
\textit{uninterruptible}~\cite[Section~4.2,~Subsection~``The~Process~Structure'']{freebsd-book}.
...
\textit{deadlocks}~\cite[Section~6.8.1]{silberschatz}. A deadlock occurs when a
group of two or more threads is stuck, with each thread in the group waiting for
a resource that is held by another thread in the group.
...
A bit more formally, the deadlock occurs because of a \textit{circular
  wait}~\cite[Section~8.3.1]{silberschatz}. A circular wait happens whenever
there is a set of threads $\{T_1, T_2,\ldots,T_n\}$ such that $T_k$ is waiting for
a resource held by $T_{k+1}$ for all $k \in \{1,\ldots,n-1\}$, and $T_n$ is
waiting for a resource held by $T_1$.
...
acquired according to that ordering~\cite[Section~8.5.4]{silberschatz}. The
ordering must be free of cycles. This freedom from cycles ensures that a
circular wait can never happen. For instance, if we imposed an ordering that
placed \Verb{mtx_a} before \Verb{mtx_b}, it would be illegal to acquire
...
ordering. This ordering is defined on lock \textit{classes}. A lock class
encompasses locks which are the same member of a certain kernel data type. For
instance, the \Verb{p_lock} mutexes of all \Verb{proc_t} structures constitute a
lock class. Global locks are in a class containing only that lock. Note that an
ordering on lock classes can be extended into an ordering on lock instances by
making locks within the same class incomparable, and comparing classes when
comparing locks from different classes. This order on instances can be extended
by rules that order locks within a single class, as long as the ordering within
each class remains free of cycles.

During the integration of the terminal subsystem with other subsystems of the
kernel (e.g.\ signal handling), the lock class of the per-terminal
...
be placed somewhere in the ordering of all lock classes. This posed some
...
happens inside the \Verb{proc_continue()} (see Listing~\ref{lst:proc-continue})
function, which is called from \Verb{sig_kill()} (see
Listing~\ref{lst:sig-kill}). The code of the \Verb{wait4()} system call (which
implements the \Verb{waitpid()} POSIX function) used \Verb{all_proc_mtx} to
synchronize with \Verb{sig_kill()}, so if we were to remove \Verb{all_proc_mtx}
from \Verb{sig_kill()}, we would also need to adjust the \Verb{wait4()} system
call's code.
...
\begin{quote}
  \textit{The \Verb{p_lock} of the parent of process \Verb{p} may be acquired
    while holding the \Verb{p_lock} of \Verb{p}, but not the other way around.}
\end{quote}
Since the process hierarchy forms a tree, there are no cycles. Therefore, we can
extend the existing ordering of lock instances (induced by the ordering of lock
classes) by the aforementioned rule while still preventing deadlocks. We could
extend this rule to arbitrary ancestors instead of just the parent, but
currently it does not bring any additional benefits.
...
\subsection{Working around lack of \Verb{select()}/\Verb{poll()}}\label{chap:tetris-workaround}
...
\Verb{setitimer()} function, and call the \Verb{read()} function to read input
...
is a screenshot of \Verb{tetris} running under Mimiker in an emulated
environment.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/tetris}
  \caption{Screenshot of \Verb{tetris} running under Mimiker on a MIPS Malta
    board emulated using QEMU.}
  \label{fig:tetris}
\end{figure}
...
When reading FreeBSD source code, the author found and reported three bugs in
the FreeBSD kernel's terminal subsystem. The FreeBSD operating system is a
widely used production-grade OS. The second bug can lead to a kernel panic
(i.e.\ a system crash), while the last one allows any process with write access
to a terminal to prevent all other processes in the system from writing to that
terminal, regardless of their credentials.
\begin{itemize}
\item Bug 250701 - ``Race condition between \Verb{tty_wait_background()} and
  ...
  Recall from Section~\ref{chap:posix-terminals} that when a process in a
  background process group attempts to read or write to its controlling
  terminal, the process group containing the offending process may be sent a
  \Verb{SIGTTIN} or \Verb{SIGTTOU} signal. This bug could lead to the offending
  process not being sent the signal if its process group is concurrently
  changed.
\item Bug 251915 - ``TOCTOU race between \Verb{tty_signal_sessleader()} and
  ...
  This bug was caused by the \Verb{tty_signal_sessleader()} function not holding
  the required lock when reading a session structure's pointer to the leader
  process. If the value of the pointer is not \Verb{NULL}, a signal is sent to
  the process. However, since the required lock is not held, the value of the
  pointer can be changed to \Verb{NULL} immediately after the check, leading to
...
\item Bug 255816 - ``tty: Potential DoS on terminal writes by stopping thread inside \Verb{ttydisc_write()}''\\
  ...
  FreeBSD serializes \Verb{write()} calls on terminals in the same way as
  Mimiker (see Listing~\ref{lst:tty-write}). Obviously, Mimiker copies this
  behaviour from FreeBSD, not the other way around. However, due to the way
  FreeBSD handles sending stop signals to threads sleeping interruptibly (see
  Section~\ref{chap:sleep-stop}), a thread can become stopped while having
  exclusive access to the terminal. This prevents all other threads from writing
  to the terminal.
...
Mimiker, as well as some shell commands that demonstrate the functionality
implemented by the author. The reader is encouraged to compile the system
themselves and test the system's capabilities using the supplied shell commands.
The \Verb{git} and \Verb{docker} programs are assumed to be installed on the
reader's system.
...
active window. We can now run various commands inside Mimiker. To close the
session, change to the pane running QEMU by pressing Control-B followed by 1, and then
press Control-C to terminate the QEMU process.
...
\item List current terminal settings:\\
  ...
  flag is not set. This means that background processes should be able to write
  to the terminal.
...
\subsubsection*{Stopping/resuming shell jobs and running \Verb{tetris}}

\begin{enumerate}
\item List current terminal settings:\\
  \Verb{stty -a}\\
  The output contains \Verb{susp = ^Z} and \Verb{isig}, which means that the
  kernel will send a \Verb{SIGTSTP} signal to the terminal's foreground process
  group upon reception of the \Verb{^Z} character (i.e. Control-Z).
\item Run the \Verb{tetris} command to play Tetris. The game's interface from
  Figure~\ref{fig:tetris} should be displayed, with game instructions visible at
  the bottom of the screen.
\item With the game running, type \Verb{^Z}. The game will stop, and the shell
  will return to the foreground.
\item Run the \Verb{jobs} command to display all jobs that the shell is aware
  of. It will display the \Verb{tetris} job and indicate its status (stopped).
\item To resume the stopped \Verb{tetris} job, run the \Verb{fg} command.\\
  Note: because of the workaround described in \ref{chap:tetris-workaround}, the
  game will resume only after the user presses any key.
\end{enumerate}

\subsubsection*{Changing control characters using \Verb{stty}}

\begin{enumerate}
\item List current terminal settings:\\
  \Verb{stty -a}\\
  The output contains \Verb{intr = ^C} and \Verb{isig}, which means that the
  kernel will send a \Verb{SIGINT} signal to the terminal's foreground process
  group upon reception of the \Verb{^C} character (i.e. Control-C).
\item To ensure that \Verb{^C} does what it's supposed to do, run the \Verb{cat}
  command without any arguments. The \Verb{cat} command reads characters from
  standard input (i.e.\ the terminal), and writes them to standard output (which
  is also the terminal in this case). The reader can confirm this by typing a
  line of text and pressing the Enter key: the line will be output by the
  process. After typing \Verb{^C}, however, the process terminates.
\item Change the \Verb{VINTR} control character to \Verb{x} using the following command:\\
  \Verb{stty intr x}\\
  The reader can verify that the control character has been changed by running
  \Verb{stty -a} again.
\item Run the \Verb{cat} command again. This time, typing \Verb{^C} does not
  terminate the process, but typing \Verb{x} does.
\end{enumerate}
