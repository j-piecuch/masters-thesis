...
access to the controlling terminal revoked. If a controlling terminal disappears
from the system (e.g.\ due to the terminal device no longer being available),
the leader of the associated session is sent a \Verb{SIGHUP} signal.
...
special to the OS and appear as input on the terminal.
...
it notifies the terminal driver of new data using \Verb{tty_notify_out()}.
...
guarantee that data will eventually arrive.
...
\item A thread sleeping interruptibly can be indefinitely prevented from waking
  ...
  all other processes from writing to a terminal.
\end{itemize}
...
\Verb{write()} system call on a terminal attempts to sleep interruptibly while
having a pending \Verb{SIGSTOP} signal:
\begin{enumerate}
